...
void map_database::register_keyframe(camera_database* cam_db, orb_params_database* orb_params_db, bow_vocabulary* bow_vocab,
    ...
    // Pose information - FIXED: Now reading world-to-camera pose (matching keyframe_trajectory.txt)
    const Mat33_t rot_wc = convert_json_to_rotation(json_keyfrm.at("rot_wc"));
    const Vec3_t trans_wc = convert_json_to_translation(json_keyfrm.at("trans_wc"));
    const auto pose_wc = util::converter::to_eigen_pose(rot_wc, trans_wc);
    const auto pose_cw = util::converter::inverse_pose(pose_wc);
