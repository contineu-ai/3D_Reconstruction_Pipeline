...
nlohmann::json keyframe::to_json() const {
    ...
    return {{"ts", timestamp_},
            ...
            // camera pose - FIXED: Store world-to-camera pose to match keyframe_trajectory.txt
            {"rot_wc", convert_rotation_to_json(pose_wc_.block<3, 3>(0, 0))},
            {"trans_wc", convert_translation_to_json(pose_wc_.block<3, 1>(0, 3))},
